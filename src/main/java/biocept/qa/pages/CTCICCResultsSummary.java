...
public class CTCICCResultsSummary extends BioceptBase{
	...
	public void ctcResult (){
		try{
		WebElement optionsText = ExplicitWait.waitUntilElementToBeVisible(ctcResultSection).findElement(By.cssSelector("select[data-bind*='optionsText']"));
	...
	public void ctcTestImage (){
		...
		ExplicitWait.waitUntilElementToBeVisible(ctcImageButton).click();
		...
		try {
			for(int i=0;i<iccImageButton.size();i++){
				...
				for(int j=i+1;j<iccChooseImages.size();j++){
					...
					ExplicitWait.waitUntilElementToBeClickable(ChooseImagesElement).click();
				}
...
//					}}
					...
					for( int k=0;k<iccSelectImages.size();k++){
						...
						for(int l=i+1;l<iccClose.size();l++){
							...
							ExplicitWait.waitUntilElementToBeClickable(ICCCloseElement).click();
							
							WebDriverWait wait_fadeIn=new WebDriverWait(driver, 10);
